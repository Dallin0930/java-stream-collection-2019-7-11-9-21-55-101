...
//                sorted(reversed ? Comparator.reverseOrder() : Comparator.naturalOrder()).collect(Collectors.toList());


        int max= Math.max(left,right);
        int min =Math.min(left,right);
        return IntStream.rangeClosed(min,max).boxed().sorted().collect(Collectors.toList());
    }
